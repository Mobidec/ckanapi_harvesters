...
class CsvFileFormat(FileFormatABC):
    default_csv_file_upload_read_csv_kwargs = dict(dtype=str, keep_default_na=False, sep=None, engine='python')  # read
    default_csv_file_download_to_csv_kwargs = df_download_to_csv_kwargs  # write
    ...
    def append_file(self, df: Union[pd.DataFrame, ListRecords], file_path: str,
        ...
        df.to_csv(file_path, index=False, mode='a', **self.write_kwargs)

    def write_in_memory(self, df: pd.DataFrame, fields: Union[Dict[str, CkanField],None]) -> bytes:
        buffer = io.StringIO()
        df.to_csv(buffer, index=False, **self.write_kwargs)
    ...
    def append_in_memory(self, buffer: bytes, df: Union[pd.DataFrame, ListRecords], fields: Union[Dict[str, CkanField],None]) -> bytes:
        ...
        df.to_csv(buffer, index=False, mode='a', **self.write_kwargs)
        return buffer.getvalue().encode("utf8")
