...
class GeoParquetFileFormat(FileFormatABC):
    ...
    def read_file(self, file_path: Union[str,io.BytesIO], fields: Union[Dict[str, CkanField],None], allow_chunks:bool=True) -> Union[pd.DataFrame, ListRecords, Iterable[pd.DataFrame], Iterable[ListRecords]]:
        ...
        for field_name in geo_columns:
            ...
            if field_data_type == "geometry" or field_data_type.startswith("geometry("):  # and field.internal_attrs.geometry_as_source:
                if epsg_target is not None:
                    # EPSG is determined by the field metadata
    ...
    def read_buffer_full(self, buffer: io.BytesIO, fields: Union[Dict[str, CkanField],None]) -> Union[pd.DataFrame, ListRecords, Iterable[pd.DataFrame], Iterable[ListRecords]]:
    ...
    def downloaded_df_to_gdf(self, df: pd.DataFrame, *, fields: Union[Dict[str, CkanField],None], context:str=None) -> gpd.GeoDataFrame:
        ...
        if self.download_conversion == GeoFileStoreEpsgConversion.CsvWkb:
