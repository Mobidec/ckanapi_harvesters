...
class ApiKey:
    ...
    def load_apikey(self, apikey_file:str=None, *, base_dir:str=None, error_not_found:bool=True, verbose:bool=False) -> bool:
        """
        ...
        if verbose:
            print(f"API key loaded from file: {apikey_file}")
        return True
    ...
    def __init__(self, apikey:str=None, *, remote_url:str=None, apikey_file:str=None, apikey_auto_load:bool=True,
        ...
        if apikey_auto_load:
            if self.is_empty() and apikey_file is not None:
                self.load_apikey(error_not_found=False, verbose=verbose)
            if self.is_empty():
                if self.load_from_environ(error_not_found=False, empty_warning=False) and verbose:
