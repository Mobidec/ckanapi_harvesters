...
class BuilderDataStoreFileHeadABC(BuilderDataStoreABC, ABC):
    """
    This first implementation only manages the first chunk of a DataFrame
    """
    ...
    def copy(self, *, dest=None):
        #if dest is None:
        #    dest = BuilderDataStoreFileHeadABC()
    ...
    def get_local_df_chunk_generator(self, resources_base_dir:str, **kwargs) -> Generator[Tuple[GeneralDataFrame,int], None, None]:
        ...
        with self.local_file_format.read_file(self.sample_data_source, fields=self._get_fields_info()) as df_file:
